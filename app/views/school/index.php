    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="align-items: center;">
                <a class=" btn btn-danger" href="<?= URLROOT; ?>/schoolsController/delete/<?= $school->schoolId ?>"
                    onclick="return confirm('Are you sure you want to delete this school?')">Delete</a>
                <a class="btn btn-info"
                    href="<?= URLROOT; ?>/schoolsController/update/<?= $school->schoolId ?>">Update</a>
            </td>
        </tr>
        <?php } ?>
    </tbody>
...
<nav aria-label="Page navigation example">
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item <?= ($data['previousPage'] <= 0) ? 'disabled' : ''; ?>">
            <a class="page-link"
                href="<?= URLROOT ?>/ManufacturersController/manufacturersOverview/<?= $data['previousPage'] ?>">Previous</a>
        </li>
        ...
        <li class="page-item <?= ($data['nextPage'] > $data['totalPages']) ? 'disabled' : ''; ?>">
            <a class="page-link"
                href="<?= URLROOT ?>/ManufacturersController/manufacturersOverview/<?= $data['nextPage'] ?>">Next</a>
        </li>
    </ul>
</nav>
